...
const claimPointMapper = require('./point-history.response-schema');
const ClaimPointStatus = require('app/model/wallet/value-object/point-status');
...
module.exports = {
  ...
  check: async (req, res, next) => {
    try {
      ...
      const survey = await surveyHelper.getInProcessSurvey(msPointSurveyIsEnabled, req.user.id);
      if (survey) {
        const points = surveyHelper.getSurveyPoint(survey, membershipType.name);
